...
export const handler = async (event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> => {
  ...
  if (path === '/local' && method === 'POST') {
    const user: { email?: string, password?: string, passwordConfirmation?: string} = JSON.parse(event.body || '{}');
    ...
    if (!existingUser) {
      try {
        ...
        return {
          ...
          headers: {
            'Set-Cookie': [
              `httpOnlyToken=Token ${token}; HttpOnly; Expires=${new Date(Date.now() + 1000 * 60 * 60 * 24 * 30).toUTCString()}`,
              `token=Token ${token}; Expires=${new Date(Date.now() + 1000 * 60 * 60 * 24 * 30).toUTCString()}`
            ]
          },
        ...
        return {
          statusCode: 500,
          body: JSON.stringify({ authentication: 'Something went wrong. Please try again later.' })
        };
      }
    }
  ...
  if (path === 'login' && method === 'POST') {
    ...
    const existingUser = await UserModel.findByEmail(user.email);
    if (existingUser && !UserModel.validatePassword(existingUser, user.password)) {
    ...
    return {
      statusCode: 422,
      body: JSON.stringify({ authentication: 'Email and password combination not recognized.' })
    }
  }
  ...
  if (path === '/current' && method === 'GET') {
    ...
    try {
      // Verify and decode token to get user._id
      ...
      return {
        ...
        headers: {
          'Set-Cookie': [
            `httpOnlyToken=Token ${newToken}; HttpOnly; Expires=${new Date(Date.now() + 1000 * 60 * 60 * 24 * 30).toUTCString()}`,
            `token=Token ${newToken}; Expires=${new Date(Date.now() + 1000 * 60 * 60 * 24 * 30).toUTCString()}`
          ]
        },
